...
class MFQEv2Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if clip_value < 108:
            qp_num = torch.tensor([0])
        elif clip_value >108 and clip_value <= 216:
            qp_num = torch.tensor([1])
        elif clip_value >216 and clip_value <= 324:
            qp_num = torch.tensor([2])
        else:
            qp_num = torch.tensor([3])
...
class VideoTestMFQEv2Dataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        # !! please change to your path !!
        self.gt_root = 'D:/Datasets/stdf_test/raw'
        self.lq_root = 'D:/Datasets/stdf_test/HEVC_QP27_3231'
